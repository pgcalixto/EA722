...
if nargin == 3           
    [mag,phase,w] = bode(sys,{wlow,whigh}, opts);
elseif nargin == 1
    [mag,phase,w] = bode(sys, opts);
end
...
plot(w/(2*pi),phase,'LineWidth',1.5);
...
plot(freqpoints(2:np-1)/(2*pi),magpoints(2:np-1),'r.-','MarkerSize',14,'LineWidth',1.3);
% No dots at start and end points:
plot(freqpoints(1:2)/(2*pi),magpoints(1:2),'r-','LineWidth',1.3);
plot(freqpoints(np-1:np)/(2*pi),magpoints(np-1:np),'r-','LineWidth',1.3);
hold off;
...
np=length(freqpoints);
plot(freqpoints(2:np-1)/(2*pi),phapoints(2:np-1),'r.-','MarkerSize',14,'LineWidth',1.3);
% No dots at start and end points:
plot(freqpoints(1:2)/(2*pi),phapoints(1:2),'r-','LineWidth',1.3);
plot(freqpoints(np-1:np)/(2*pi),phapoints(np-1:np),'r-','LineWidth',1.3);
